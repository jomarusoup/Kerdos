...
def ai_trading():
    ...
    # 24시간 동안의 1시간봉 (minute60) 24개
    df_24h = pyupbit.get_ohlcv("KRW-ETH", count=24, interval="minute60")
    # 오더북(호가 정보)
    orderbook = pyupbit.get_orderbook("KRW-ETH")  # KRW-ETH 매수/매도 호가

    # 아직 로그인 전이므로, 투자 상태(잔고)는 아래에서 upbit 객체 만들어서 조회
    #---------------------------------------------------
    ...
    # 아래는 예시로, GPT에게 넘길 딕셔너리를 생성해 JSON으로 변환
    # (데이터가 많을 수 있으니, 필요시 축약/가공해서 보내는 방법도 고려하세요.)
    data_for_gpt = {
        "chart_data_30d": df_30d.to_dict(),      # df를 to_json()해도 됨
    ...
    # 현재 투자 상태(잔고) 조회
    my_krw = upbit.get_balance("KRW")       # 보유 원화
    my_eth = upbit.get_balance("KRW-ETH")   # 보유 ETH
    print(f"[투자상태] 현재 보유 KRW: {my_krw}, 보유 ETH: {my_eth}")

    # 콘솔에 ChatGPT 결정/사유 출력
    decision = gpt_result.get("decision", "hold")  # 기본값 hold
    reason   = gpt_result.get("reason", "No reason provided.")
